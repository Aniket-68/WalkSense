...
const CAMERA_FEED_URL = `${API_BASE}/api/camera/feed`;
    ...
    <div className="camera-container">
      {isRunning ? (
        <>
          ...
          {(description || spatialSummary) && (
            <div className="scene-description">
              {spatialSummary && (
                <div className="label">Spatial Sense</div>
              )}
              {spatialSummary && <div style={{ marginBottom: 4, fontSize: '0.75rem', color: 'var(--text-secondary)' }}>{spatialSummary}</div>}
        ...
        <div className="camera-placeholder">
          ...
          <p>Camera feed offline — press <strong>Start</strong> to begin</p>
        </div>
      )}
    </div>
  );
