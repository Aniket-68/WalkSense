    ...
    <div className="app-layout">
      ...
      <header className="header">
        <div className="header-title">
          <div className={`logo-dot ${systemStatus === 'RUNNING' ? 'animate-pulse-glow' : ''}`} />
          <h1>WalkSense</h1>
          <span className="badge" style={{
            background: systemStatus === 'RUNNING' ? 'var(--accent-green-dim)' : 'var(--bg-card)',
            color: systemStatus === 'RUNNING' ? 'var(--accent-green)' : 'var(--text-muted)',
            border: systemStatus === 'RUNNING' ? '1px solid rgba(0,200,83,0.3)' : '1px solid var(--border-subtle)',
          }}>
            {systemStatus}
          </span>
        </div>
        ...
        <div className="header-status">
          ...
          <KeyboardShortcuts onStartStop={handleStartStop} onListen={handleListen} />
        </div>
      </header>
